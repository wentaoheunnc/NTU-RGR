...
for i = 1:98 %??????
    mkdir('dataspec_pro2\train\', num2str(i));
    display(int2str(i));
    tic;
    t_old = t;
...
%                 tmp2(:,:,1) = tmp;
                imwrite(uint8(tmp), ['dataspec_pro2\train\' num2str(i) '\' num2str(t) '.PNG']);
%                  figure;imshow(tmp);
                 t = t + 1;
            end
...
for i = 1:98
    mkdir('dataspec_pro2\test\', num2str(i));
    display(int2str(i));
    tic;
    t_old = t;
    test_idx = test_n{i};
...
%                 tmp2 = zeros(115,115,3);
%                 tmp2(:,:,1) = tmp;
                imwrite(uint8(tmp), ['dataspec_pro2\test\' num2str(i) '\' num2str(t) '.PNG']);
